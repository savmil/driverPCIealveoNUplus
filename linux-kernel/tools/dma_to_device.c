		    ...
		    char *ofname)
{
	...
	if( infname) {
		...
		posix_memalign((void **)&allocated, 4096  , size_file + 4096);
	if (!allocated) {
		fprintf(stderr, "OOM %lu.\n", size + 4096);
		rc = -ENOMEM;
		goto out;
	}
	char * appoggio=malloc(sizeof(char)*2);
	...
	printf("qui\n");
		...
		{
			line_pos=12;
			while (line_pos<read-2)
			{
				
				line_pos++;
				appoggio[0]=line[line_pos];
				line_pos++;
				appoggio[1]=line[line_pos];
				//printf("line %c\n",line[line_pos]);
				buffer[pos]=strtol(appoggio,NULL,16);
				pos++;
				//printf("line hex %02X\n",buffer[pos]);
			}
			//sleep(1);
	...
	buffer = allocated + offset;*/ //vecchia lettura binaria
	if (verbose)
	...
	for (i = 0; i < count; i++) {
		...
		rc = write_from_buffer_swapped_file(devname, fpga_fd, buffer, size_file, addr);
		if (rc < 0)
			goto out;
